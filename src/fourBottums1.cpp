    ...
    while(!done){
        ...
        if(peekmessage(&m,EM_MOUSE|EM_KEY)){
            ...
            if(m.message==WM_LBUTTONDOWN){
                ...
                if(inside(m.x,m.y,cx,cy+210,150,40)) break;
            }
        }
        Sleep(16); 
    }
        ...
        if(peekmessage(&m,EM_MOUSE|EM_KEY)){
            if(m.message==WM_KEYDOWN){
                ...
                if(inside(m.x,m.y,cx,cy+70,100,40) && len>0){
                    *val=atof(buf);
                    finish=true;  //点击了OK键
                }else if(inside(m.x,m.y,cx+130,cy+70,100,40)) finish=true;
            }
        }
    }
    restoreScr();
...
void updBtns(){
    ...
    while(peekmessage(&m,EM_MOUSE)){
        if(m.message==WM_LBUTTONDOWN){
            if(inside(m.x,m.y,btnx,colrBtnY,btnW,BtnH)) colorSelect();
            else if(inside(m.x,m.y,btnx,gravBtnY,btnW,BtnH)) numInput("Gravity",&gravVal);
            else if(inside(m.x,m.y,btnx,fricBtnY,btnW,BtnH)) numInput("Friction",&fricVal);
            else if(inside(m.x,m.y,btnx,spdBtnY,btnW,BtnH)) numInput("Speed",&speedVal);
        }
    }
}
