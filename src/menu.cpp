...
// Left/Right - change numeric parameters
// Enter - start demo / confirm
// R - reset demo to initial state
// Esc - quit
// Space - in demo: pause / resume
// Any other key: in demo, restart run (also acts like your 'button')

enum class AppState { MENU, DEMO_RUNNING, DEMO_PAUSED };
enum class ModelType { SLOPE = 0, COLLISION = 1 };

struct SlopeParams {
    // ramp geometry in meters (we'll set in world coordinates)
    double top_x, top_y;
...
int main() {
    ...
    const int WINW = 1024;
    const int WINH = 720;
    const double SCALE = 60.0; // px per meter
    ...
    slope.mu = 0.15;
    slope.ball.radius = 0.18; // m
    slope.ball.mass = 0.5;
    slope.ball.color = RED;
    slope.ball.vx = slope.ball.vy = 0.0;
    // ball initial pos will be placed at ramp top (slightly offset above ramp)
    ...
    // helper lambdas
    auto ramp_angle = [&](const RampData& r)->double {
        double dx = r.x2 - r.x1;
        double dy = r.y2 - r.y1;
        return atan2(dy, dx); // radians
        };

    // Build RampData from slope params
    auto buildRamp = [&](const SlopeParams& sp)->RampData {
        RampData r;
        r.x1 = sp.top_x; r.y1 = sp.top_y;
        r.x2 = sp.bottom_x; r.y2 = sp.bottom_y;
        r.mu = sp.mu;
        return r;
        };

    // initial copies to enable reset
    ...
    int menuIndex = 0; // 0: model select, 1: param1, 2: param2, 3: start
    int maxMenu = 3;
    ...
    while (!quit) {
        ...
        if (state == AppState::MENU) {
            ...
            // Draw placeholder start button
            renderer.DrawButtonPlaceholder(30, 320, 160, 44, "Start Demo (Enter)");
            renderer.DrawButtonPlaceholder(210, 320, 160, 44, "Reset (R)");
            renderer.DrawButtonPlaceholder(390, 320, 160, 44, "Pause (Space)");
            renderer.DrawButtonPlaceholder(570, 320, 160, 44, "Quit (Esc)");

            // Draw preview area (right side)
            int previewX = 30;
            int previewY = 380;
            int previewW = 800;
            int previewH = 300;
            ...
            // Draw preview world background
            // For preview, draw one ramp or two balls depending on model
            if (model == ModelType::SLOPE) {
                RampData rp = buildRamp(slope);
                renderer.DrawRamp(rp);
                // position ball at top for preview
            ...
            if (_kbhit()) {
                int ch = _getch();
                if (ch == 0 || ch == 0xE0) { // arrow keys or extended
                    int ch2 = _getch();
                    if (ch2 == 72) { // up
                        menuIndex = std::max(0, menuIndex - 1);
                    }
                    else if (ch2 == 80) { // down
                        menuIndex = std::min(maxMenu, menuIndex + 1);
                    }
                    else if (ch2 == 75) { // left
                        // change parameter decrease
                        if (model == ModelType::SLOPE) {
