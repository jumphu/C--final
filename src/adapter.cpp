...
bool PhysicsVisualAdapter::initialize(int screenWidth, int screenHeight) {
    ...
    initSceneModelButtons(sceneBtnX, s1Y, s2Y, s3Y, s4Y, s5Y, sW, sH, 20); 
...
void PhysicsVisualAdapter::renderFrame() {
    ...
    for (const auto& pair : objectConnections) {
        ...
        switch (conn.type) {
            case OBJ_CIRCLE: {
                ...
                if (GetRValue(conn.color) + GetGValue(conn.color) + GetBValue(conn.color) < 300) {
                    textColor = WHITE;
                }
    ...
    drawSceneModelButtons(btnLayout.sceneBtnX, btnLayout.s1Y, btnLayout.s2Y, btnLayout.s3Y, btnLayout.s4Y, btnLayout.s5Y, btnLayout.sW, btnLayout.sH);
...
void PhysicsVisualAdapter::handleButtonClicks() {
    ...
    // Scene buttons
    if (getSphereCreationButtonState()) { // Slope Scene
        switchScene(SCENE_SINGLE_OBJECT); 
        extern bool sphere_creationClicked; sphere_creationClicked = false;
    }
    if (getTwoStarsButtonState()) { // Ball Collision
        switchScene(SCENE_TWO_OBJECTS);
        extern bool two_starsClicked; two_starsClicked = false;
    }
    if (getSolarSysButtonState()) { // Stacking Demo
        switchScene(SCENE_SPHERE_CREATION);
        extern bool solar_sysClicked; solar_sysClicked = false;
    }
    if (getBlockSlopeButtonState()) { // Slope Left
        switchScene(SCENE_SOLAR_SYS); 
        extern bool block_slopeClicked; block_slopeClicked = false;
    }
    if (getBlockOnBoardButtonState()) { // Block on Board
        switchScene(SCENE_BLOCK_ON_BOARD);
        extern bool block_on_boardClicked; block_on_boardClicked = false;
    }
}
...
void PhysicsVisualAdapter::onMouseClick(int screenX, int screenY, bool leftButton) {
    ...
    if (isInButton(screenX, screenY, btnLayout.sceneBtnX, btnLayout.s5Y, btnLayout.sW, btnLayout.sH)) {
        extern bool block_on_boardClicked; block_on_boardClicked = true; return;
    }
...
void PhysicsVisualAdapter::onMouseMove(int screenX, int screenY) {
    ...
    extern bool sphere_creationHovered, two_starsHovered, solar_sysHovered, block_slopeHovered, block_on_boardHovered;
    ...
    block_on_boardHovered = isInButton(screenX, screenY, btnLayout.sceneBtnX, btnLayout.s5Y, btnLayout.sW, btnLayout.sH);
}
...
void PhysicsVisualAdapter::onKeyPress(char key) {
    switch (key) {
        ...
        case '4': switchScene(SCENE_SOLAR_SYS); break; 
        case '5': switchScene(SCENE_BLOCK_ON_BOARD); break;
    }
}
...
void PhysicsVisualAdapter::initializeScene(SceneMode scene) {
    ...
    switch (scene) {
        ...
        case SCENE_SOLAR_SYS:
             // Scene 4: Ball Slope Left (Left Low, Right High)
             physicsWorld->setInclineAngle(0.0);
             createPhysicsObject(OBJ_SLOPE, 0, 2, 16.0, 0.26, 1000.0, RGB(100, 100, 100), false);
             // Ball at Top-Right
             // Slope P(7.5, 3.995). Normal N(-0.257, 0.966). Center = (7.243, 4.961)
             createPhysicsObject(OBJ_CIRCLE, 7.24, 4.96, 1.0, 0.0, 1.0, RGB(0, 0, 255), true);
             break;
             
        case SCENE_BLOCK_ON_BOARD:
             // Scene 5: Block on Board
             physicsWorld->setInclineAngle(0.0);
