    ...
    while(!done) {
        ...
        while(peekmessage(&m,EM_MOUSE|EM_KEY)) {
            ...
            if(m.message==WM_LBUTTONDOWN) {
                ...
                if(inside(m.x,m.y,cx,cy+210,150,40)) { // Cancel
                    done=true;
                    break;
                }
            }
    ...
    while(!finish) {
        ...
        if(peekmessage(&m, EM_MOUSE | EM_KEY)) {
            ...
            if(m.message == WM_KEYDOWN) {
                if(m.vkcode == VK_RETURN) { // Enter
                    if(len > 0) *val = (float)atof(buf);
                    finish = true;
                }
                else if(m.vkcode == VK_ESCAPE) { // ESC
                    finish = true;
                }
            }
            // 2. 处理字符输入 (WM_CHAR) - 这才是输入数字的正确方式
            else if(m.message == WM_CHAR) {
                if(m.ch == 8) { // Backspace (ASCII 8)
                    if(len > 0) {
                        len--;
                        buf[len] = '\0';
                    }
            ...
            // 3. 处理鼠标点击
                ...
                if(inside(m.x, m.y, cx+30, cy+70, 70, 40)) { // OK
                    if(len > 0) *val = (float)atof(buf);
                    finish = true; 
                }
