...
/*=========================================================================================================
 * Pause() - 暂停物理模拟
 * 修改：只设置标志位，不要保存状态，否则拖拽会被覆盖
 *=========================================================================================================*/
void PhysicalWorld::Pause() {
    if (!isPaused) {
        // saveStates(); // <--- 【注释掉或删除这一行】不要保存状态
        isPaused = true;
...
/*=========================================================================================================
 * Continue() - 继续物理模拟
 * 修改：只恢复标志位，不要恢复状态，保留用户的拖拽修改
 *=========================================================================================================*/
void PhysicalWorld::Continue() {
    if (isPaused) {
        // restoreStates(); // <--- 【注释掉或删除这一行】凶手就是它！删掉！
        isPaused = false;
...
void PhysicalWorld::handleAllCollisions(std::vector<Shape*>& shapeList) {
    ...
    // 2. 【新增】检测动态物体与静态物体的碰撞 (Dynamic vs Static)
    // 遍历每一个动态物体
    for (size_t i = 0; i < shapeList.size(); i++) {
        // 遍历每一个静态物体 (墙壁、静态斜坡等)
        for (size_t k = 0; k < staticShapeList.size(); k++) {
            ...
            // 距离检查 (优化性能)
            double x1, y1, x2, y2;
            dynamicShape->getCentre(x1, y1);
            staticShape->getCentre(x2, y2);
            
            // 墙壁可能很大，简单的中心距离检查可能不准，
            // 但为了防止漏检，这里稍微放宽距离限制，或者直接检测
            // 对于墙壁这种大型物体，最好直接调用 check_collision
            
            // 碰撞检测
            if (dynamicShape->check_collision(*staticShape)) {
                // 如果是墙壁，调用专门的墙壁处理函数效果更好，但通用的 resolveCollision 也能处理无穷大质量
                // 这里优先尝试转换成 Wall 指针来处理，如果不是 Wall 则走通用逻辑
                
                Wall* wall = dynamic_cast<Wall*>(staticShape);
                if (wall) {
                    resolveCollisionWithWall(*dynamicShape, *wall);
                } else {
                    // 比如静态的 Slope 或者其他 StaticShape
