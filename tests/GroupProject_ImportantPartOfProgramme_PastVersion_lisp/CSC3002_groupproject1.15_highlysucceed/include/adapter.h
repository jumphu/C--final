...
#include "Renderer.h"
// 请确保这里引用的文件名和你实际硬盘上的文件名一致（你说是 allbottums.h）
...
private:
    ...
    // 【关键变量定义】之前报错uiParams未声明就是因为缺了这个
    struct UIParameters {
        float gravity = 9.8f;
        float friction = 0.1f;
    ...
    // 【关键变量定义】之前报错buttonPos未声明就是因为缺了这个
    struct ButtonPositions {
        int startBtnX, pauseBtnX, stopBtnX, btnY, btnW, btnH;
        int chooseBtnX, model1BtnY, model2BtnY, solarSysBtnY_M3, sphereCreationBtnY_M3, twoStarsBtnY_M3, chooseBtnW, chooseBtnH;
        int spaceBtnX, sphereCreationBtnY, twoStarsBtnY, solarSysBtnY, spaceBtnW, spaceBtnH;
    };
    ButtonPositions buttonPos;
