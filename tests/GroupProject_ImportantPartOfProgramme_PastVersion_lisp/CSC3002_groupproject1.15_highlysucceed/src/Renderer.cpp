...
int Renderer::WorldToScreenY(double wy) const {
    // 屏幕中心 y - 物理位移 (向上为正，但在屏幕坐标系中向下为正，所以要减)
...
// 【新增】画线函数的实现
void Renderer::DrawLine(double x1, double y1, double x2, double y2, COLORREF color, int thickness) {
    setlinecolor(color);
    setlinestyle(PS_SOLID, thickness);
    // 把物理世界坐标转换成屏幕坐标再画
    line(WorldToScreenX(x1), WorldToScreenY(y1), WorldToScreenX(x2), WorldToScreenY(y2));
}
