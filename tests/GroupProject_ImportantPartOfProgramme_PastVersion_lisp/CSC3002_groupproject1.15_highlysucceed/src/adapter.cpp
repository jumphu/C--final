...
bool PhysicsVisualAdapter::initialize(int screenWidth, int screenHeight) {
    ...
    initChooseButtons(buttonPos.chooseBtnX, buttonPos.model1BtnY, buttonPos.model2BtnY, 
                      buttonPos.solarSysBtnY_M3, buttonPos.sphereCreationBtnY_M3, buttonPos.twoStarsBtnY_M3,
                      buttonPos.chooseBtnW, buttonPos.chooseBtnH, margin);
...
void PhysicsVisualAdapter::renderFrame() {
    ...
    if (currentScene >= SCENE_SPHERE_CREATION && currentScene <= SCENE_SOLAR_SYS) {
        ...
        drawChooseButtons(buttonPos.chooseBtnX, buttonPos.model1BtnY, buttonPos.model2BtnY,
                          buttonPos.solarSysBtnY_M3, buttonPos.sphereCreationBtnY_M3, buttonPos.twoStarsBtnY_M3,
                          buttonPos.chooseBtnW, buttonPos.chooseBtnH);
    }
        ...
        case SCENE_SINGLE_OBJECT: {
            ...
            for (auto& pair : objectConnections) {
                ...
                if (conn.type == OBJ_CIRCLE && conn.physicsObject) {
                    ...
                    // 【修正】仅在暂停或停止时允许 UI 修改速度，避免运行时干扰物理加速
                    if ((!isSimulationRunning || isSimulationPaused) && fabs(currentVx - newSpeed) > 0.1f) {
            ...
            for (int id = 1; id <= nextObjectId; ++id) {
                ...
                if (conn.type == OBJ_CIRCLE && conn.physicsObject) {
                    ...
                    if (circleCount == 1) { // 左球
                        ...
                        if ((!isSimulationRunning || isSimulationPaused) && fabs(vx - s1) > 0.1f) circle->setVelocity(s1, vy);
                    }
                        ...
                        double vx, vy;
                        circle->getVelocity(vx, vy);
                        if ((!isSimulationRunning || isSimulationPaused) && fabs(vx - s2) > 0.1f) circle->setVelocity(s2, vy);
                    }
                }
            }
            break;
...
void PhysicsVisualAdapter::handleButtonClicks() {
    ...
    if (getSolarSysButtonState_M3()) {
        resetChooseButtons();
        system("start \"\" \"model-3\\solar system.bat\"");
    }
    
    if (getSphereCreationButtonState_M3()) {
        resetChooseButtons();
        system("start \"\" \"model-3\\sphere creation.bat\"");
    }
    
    if (getTwoStarsButtonState_M3()) {
        resetChooseButtons();
        system("start \"\" \"model-3\\two stars.bat\"");
    }
