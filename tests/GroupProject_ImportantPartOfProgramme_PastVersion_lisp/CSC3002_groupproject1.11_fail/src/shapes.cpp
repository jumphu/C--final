...
bool Circle::check_collision(const Shape& other) const {
    ...
    if (s) {
        ...
        double dx = mass_centre[0] - slopeX, dy = mass_centre[1] - slopeY;
        double cosA = std::cos(-slopeAngle), sinA = std::sin(-slopeAngle);
        double localX = dx * cosA - dy * sinA;
        double localY = dx * sinA + dy * cosA;
        double closestX = std::max(-halfLen, std::min(halfLen, localX));
        double closestY = 0.0; 
        double distX = localX - closestX, distY = localY - closestY;
