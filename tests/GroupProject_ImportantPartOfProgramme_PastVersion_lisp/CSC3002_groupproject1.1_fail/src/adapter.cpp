...
void PhysicsVisualAdapter::handleParameterChanges() {
    ...
    switch (currentScene) {
        case SCENE_SINGLE_OBJECT: updBtns(); break;
        case SCENE_TWO_OBJECTS:   updBtns2(); break;
        default: break;
    }
}

void PhysicsVisualAdapter::handleButtonClicks() {
    // 【重要修改】使用 reset...Buttons() 替代 resetAll
...
void PhysicsVisualAdapter::onMouseClick(int screenX, int screenY, bool leftButton) {
    ...
    // 【重要修改】调用修正后的输入处理
    handleCtrlMouse(buttonPos.startBtnX, buttonPos.pauseBtnX, buttonPos.stopBtnX,
                    buttonPos.btnY, buttonPos.btnW, buttonPos.btnH);
    
    if (currentScene >= SCENE_SPHERE_CREATION && currentScene <= SCENE_SOLAR_SYS) {
        handleSpaceMouse(buttonPos.spaceBtnX, buttonPos.sphereCreationBtnY,
                         buttonPos.twoStarsBtnY, buttonPos.solarSysBtnY,
                         buttonPos.spaceBtnW, buttonPos.spaceBtnH);
    } else {
        handleChooseMouse(buttonPos.chooseBtnX, buttonPos.model1BtnY, buttonPos.model2BtnY,
                          buttonPos.model3BtnY, buttonPos.chooseBtnW, buttonPos.chooseBtnH);
    }
...
void PhysicsVisualAdapter::onMouseMove(int screenX, int screenY) {
    // 【重要修改】调用修正后的悬停检查
    // 注意：原始代码的handleMouseInput内部同时处理了MOVE和CLICK，
    // 这里我们传入坐标主要是为了触发内部的 WM_MOUSEMOVE 逻辑。
    // 由于allbuttons系列函数内部使用 peekmessage，这里其实有点 redundancy，
    // 但为了保持状态更新，可以在主循环的 peekmessage 之后不用特意调这里的 onMouseMove，
    // 或者 adapter 内部实现得更干净点。目前保持原状调用即可。
    
    // 注意：原 handleMouseInput 是自取消息，不需要传 x,y，
    // 但这里为了触发 Adapter 结构，我们保留调用。
    // 实际上 UI 模块是自己 peekmessage 的，这里如果不传参可能需要改 UI 模块接口。
    // 鉴于 UI 模块已经改了 handle...Mouse，它们内部会处理 peekmessage。
    // 所以这里的参数 screenX, screenY 其实没用到，全靠 UI 模块内部的 message loop。
