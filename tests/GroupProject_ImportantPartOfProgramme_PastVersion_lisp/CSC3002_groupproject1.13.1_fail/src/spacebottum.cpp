...
void handleSpaceMouse(int mx, int my, bool isDown, int btnX, int sphere_creationBtnY, int two_starsBtnY, int solar_sysBtnY, int btnW, int btnH) {
    // 1. 更新悬停状态
    sphere_creationHovered = isInButton(mx, my, btnX, sphere_creationBtnY, btnW, btnH);
    two_starsHovered       = isInButton(mx, my, btnX, two_starsBtnY, btnW, btnH);
    solar_sysHovered       = isInButton(mx, my, btnX, solar_sysBtnY, btnW, btnH);
...
void initSpaceButtons(int& btnX, int& sphere_creationBtnY, int& two_starsBtnY, int& solar_sysBtnY, int& btnW, int& btnH, int margin) {
    ...
    // 2. 从 Y=100 开始向下排列
    sphere_creationBtnY = 100;
    two_starsBtnY = sphere_creationBtnY + btnH + margin;
    solar_sysBtnY = two_starsBtnY + btnH + margin;
}